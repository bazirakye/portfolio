<div x-data="{ darkMode: false }" class="justify-end hidden pr-16 sm:flex lg:pr-0">
    <div
        class="dark:bg-dark-3 shadow-1 dark:shadow-box-dark fixed z-[99999] bottom-10 right-10 flex h-11 w-11 items-center justify-center rounded bg-white">
        <label for="themeSwitcher" class="inline-flex cursor-pointer items-center" aria-label="themeSwitcher"
            name="themeSwitcher">
            <input type="checkbox" name="themeSwitcher" id="themeSwitcher" class="sr-only" x-model="darkMode"
                @click="document.documentElement.classList.toggle('dark', darkMode)">

            <span x-show="darkMode" class="block text-body-color dark:hidden">
            ...
            <span x-show="!darkMode" class="hidden text-white dark:block">
                <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
